...
`伙伴云自定义组件`
依托伙伴云的页面编辑功能，给用户提供一个可以自己定义组件元数据/数据源限制/该组件自用的附件等内容的组件，并且用户还可以自己定义组件的样式和行为、渲染等，变相对客户开放了一部分的代码编辑权限

该组件包含以下功能：
- 自定义组件元数据的编辑功能，包括组件的名称、描述、图标、样式等
- 自定义组件数据源限制的编辑功能，包括组件可以使用的数据源类型（如在线表格、手动输入等）、数据源可以选择的字段的类型和编辑界面上label名称等。
- 自定义组件自用的附件的编辑功能，包括上传、删除、查看等
- 自定义组件本身的代码编辑功能，包括组件的CSS、React组件代码 等代码的编辑和预览功能。
- 自定义组件发布功能，包括将用户编辑的组件发布到伙伴云的组件库中，使其他用户可以在伙伴云的页面编辑系统中使用该组件。
- 自定义组件渲染功能，包括组件在页面编辑系统中的展示和在页面展示系统中的展示。

包含技术栈：

- Typescript / React / Vite / Sucrase / ECharts

我在其中负责：
- 自定义组件元数据的编辑功能的设计和实现
- 自定义组件数据源限制的编辑功能的设计和实现
- 自定义组件自用的附件的编辑功能的设计和实现
- 自定义组件本身的代码编辑功能的设计和实现
- 自定义组件渲染功能的设计和实现
...
- Typescript / React / Vite / Rxjs / Websocket / ECharts
